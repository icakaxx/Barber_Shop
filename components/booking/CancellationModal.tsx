...
export default function CancellationModal() {
  ...
  const submitCancellation = () => {
    if (reason.length < 5) {
      alert(t('booking.cancelInvalidReason'));
      return;
    }
    closeModal();
  };
  ...
  return (
    <div id="cancelModal" className={`fixed inset-0 z-[110] ${isOpen ? '' : 'hidden'}`}>
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-2xl shadow-2xl p-6">
        <h3 className="text-xl font-bold mb-4">{t('booking.cancelModalTitle')}</h3>
        <div className="space-y-4">
          <div>
            ...
            />
            <p className="text-xs text-gray-400 mt-1">{t('booking.cancelMinCharsHint')}</p>
          </div>
